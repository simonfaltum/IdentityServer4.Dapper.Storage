...
namespace IdentityServer4.Dapper.Storage.DataLayer
{
    public class DefaultClientProvider : IClientProvider
    {
        ...
        public async Task<Clients> GetClientById(string clientId)
        {
            ...
            var query = $"select * from {_options.DbSchema}.Clients where ClientId = @ClientId";
            await using var connection = new SqlConnection(_connectionString);
            await connection.OpenAsync();
            return await connection.QueryFirstOrDefaultAsync<Clients>(query, new {ClientId = clientId});
        }
        ...
        public async Task AddAsync(Client client)
        {
            ...
            try
            {
                var ClientId = await con.ExecuteScalarAsync<int>(
                    ...
                    "UpdateAccessTokenClaimsOnRefresh],[Created],[Updated],[LastAccessed],[UserSsoLifetime],[UserCodeType],[DeviceCodeLifetime],[NonEditable],[AllowedIdentityTokenSigningAlgorithms],[RequireRequestObject]) " +
                    ...
                    "@LogoUri,@PairWiseSubjectSalt,@ProtocolType,@RefreshTokenExpiration,@RefreshTokenUsage,@RequireClientSecret,@RequireConsent,@RequirePkce,@SlidingRefreshTokenLifetime," +
                    $"@UpdateAccessTokenClaimsOnRefresh,@Created,@Updated,@LastAccessed,@UserSsoLifetime,@UserCodeType,@DeviceCodeLifetime,@NonEditable,@AllowedIdentityTokenSigningAlgorithms,@RequireRequestObject);{_options.GetLastInsertID}",
                ...
                if (client.AllowedGrantTypes != null)
                {
                    foreach (var item in client.AllowedGrantTypes)
                    {
                        result = await con.ExecuteAsync(
                            $"insert into {_options.DbSchema}.ClientGrantTypes (ClientId,GrantType) values (@ClientId,@GrantType)",
                            new
                            {
                                ClientId,
                                GrantType = item
                ...
                if (client.RedirectUris != null)
                {
                    ...
                    {
                        result = await con.ExecuteAsync(
                            $"insert into {_options.DbSchema}.ClientRedirectUris (ClientId,RedirectUri) values (@ClientId,@RedirectUri)",
                            new
                            {
                                ClientId,
                                RedirectUri = item
                ...
                if (client.PostLogoutRedirectUris != null)
                {
                    ...
                    {
                        result = await con.ExecuteAsync(
                            $"insert into {_options.DbSchema}.ClientPostLogoutRedirectUris (ClientId,PostLogoutRedirectUri) values (@ClientId,@PostLogoutRedirectUri)",
                            new
                            {
                                ClientId,
                        ...
                        {
                            throw new Exception(
                                $"Error inserting into ClientPostLogoutRedirectUris, return value is {result}");
                        }
                    }
                }
                ...
                if (client.AllowedScopes != null)
                {
                    ...
                    {
                        result = await con.ExecuteAsync(
                            $"insert into {_options.DbSchema}.ClientScopes (ClientId,Scope) values (@ClientId,@Scope)",
                            new
                            {
                                ClientId,
                                Scope = item
                ...
                if (client.ClientSecrets != null)
                {
                    ...
                    {
                        result = await con.ExecuteAsync(
                            $"insert into {_options.DbSchema}.ClientSecrets (ClientId,Description,Expiration,[Type],[Value], Created) values (@ClientId,@Description,@Expiration,@Type,@Value, @Created)",
                            new
                            {
                                ClientId,
                                item.Description,
                ...
                if (client.Claims != null)
                {
                    ...
                    {
                        result = await con.ExecuteAsync(
                            $"insert into {_options.DbSchema}.ClientClaims ([ClientId],[Type],[Value]) values (@ClientId,@Type,@Value)",
                            new
                            {
                                ClientId,
                                item.Type,
                ...
                if (client.IdentityProviderRestrictions != null)
                {
                    ...
                    {
                        result = await con.ExecuteAsync(
                            $"insert into {_options.DbSchema}.ClientIdPRestrictions (ClientId,Provider) values (@ClientId,@Provider)",
                            new
                            {
                                ClientId,
                                Provider = item
                        ...
                        {
                            throw new Exception(
                                $"Error inserting into ClientIdPRestrictions, return value is {result}");
                        }
                    }
                }
                ...
                if (client.AllowedCorsOrigins != null)
                {
                    ...
                    {
                        result = await con.ExecuteAsync(
                            $"insert into {_options.DbSchema}.ClientCorsOrigins (ClientId,Origin) values (@ClientId,@Origin)",
                            new
                            {
                                ClientId,
                                Origin = item
                ...
                if (client.Properties != null)
                {
                    ...
                    {
                        result = await con.ExecuteAsync(
                            $"insert into {_options.DbSchema}.ClientProperties (ClientId,[Key],[Value]) values (@ClientId,@Key,@Value)",
                            new
                            {
                                ClientId,
                                item.Key,
        ...
        {
            await using var connection = new SqlConnection(_connectionString);
            var corsOrigins = await connection.QueryAsync<string>(
                $"select distinct(Origin) from {_options.DbSchema}.[ClientCorsOrigins] where Origin is not null",
        ...
        public async Task RemoveAsync(string clientId)
        {
            ...
            try
            {
                var ret = await con.ExecuteAsync($"delete from {_options.DbSchema}.Clients where id=@id",
                    new {clientEntity.Id}, commandTimeout: _options.CommandTimeOut, commandType: CommandType.Text,
                    transaction: t);
                ret = await con.ExecuteAsync(
                    $"delete from {_options.DbSchema}.ClientGrantTypes where ClientId=@ClientId;" +
        ...
        private async Task<IEnumerable<ClientGrantTypes>> GetClientGrantTypeByClientId(int ClientId, IDbConnection con,
            IDbTransaction t)
        {
            var result = await con.QueryAsync<ClientGrantTypes>(
                $"select * from {_options.DbSchema}.ClientGrantTypes where  ClientId = @ClientId", new {ClientId},
                commandTimeout: _options.CommandTimeOut, commandType: CommandType.Text, transaction: t);
            return result;
        }
        ...
        private async Task UpdateClientGrantTypeByClientId(IEnumerable<string> clientGrantTypes, int clientId,
            IDbConnection con, IDbTransaction t)
        {
            ...
            foreach (var grant in updatedGrants)
            {
                if (existingGrants.All(x => x.GrantType != grant))
                {
                    ...
                    await con.ExecuteAsync(insertSql, insertParameters, transaction: t);
                }
            }

            var removeTheseItems = existingGrants.Where(c => !updatedGrants.Exists(d => d == c.GrantType))
                .Select(c => c.GrantType).ToArray();
        ...
        public async Task<IEnumerable<ClientRedirectUris>> GetClientRedirectUriByClientId(int clientId,
            IDbConnection con, IDbTransaction t)
        {
            return await con.QueryAsync<ClientRedirectUris>(
                $"select * from {_options.DbSchema}.ClientRedirectUris where ClientId=@ClientId",
                new {ClientId = clientId}, commandTimeout: _options.CommandTimeOut, commandType: CommandType.Text,
                transaction: t);
        }
        ...
        private async Task UpdateClientRedirectUriByClientId(IEnumerable<string> clientRedirectUris, int clientId,
            IDbConnection con, IDbTransaction t)
        {
            ...
            var removeTheseItems = existingItems.Where(c => !updatedItems.Exists(d => d == c.RedirectUri))
                .Select(c => c.RedirectUri).ToArray();
            if (!removeTheseItems.IsNullOrEmpty())
            {
                await con.ExecuteAsync(
        ...
        public async Task<IEnumerable<ClientPostLogoutRedirectUris>> GetClientPostLogoutRedirectUriByClientID(
            int ClientId)
        {
            await using var con = new SqlConnection(_connectionString);
            await con.OpenAsync();
        ...
        public async Task<IEnumerable<ClientPostLogoutRedirectUris>> GetClientPostLogoutRedirectUriByClientID(
            int ClientId, IDbConnection con, IDbTransaction t)
        {
            return await con.QueryAsync<ClientPostLogoutRedirectUris>(
                $"select * from {_options.DbSchema}.ClientPostLogoutRedirectUris where ClientId=@ClientId",
                new {ClientId}, commandTimeout: _options.CommandTimeOut, commandType: CommandType.Text, transaction: t);
        }

        public async Task UpdateClientPostLogoutRedirectUriByClientID(IEnumerable<string> clientPostLogoutRedirectUris,
            int ClientId)
        {
            await using var con = new SqlConnection(_connectionString);
            await con.OpenAsync();
        ...
        private async Task UpdateClientPostLogoutRedirectUriByClientID(IEnumerable<string> clientPostLogoutRedirectUris,
            int clientId, IDbConnection con, IDbTransaction t)
        {
            ...
            var removeTheseItems = existingItems.Where(c => !updatedItems.Exists(d => d == c.PostLogoutRedirectUri))
                .Select(c => c.PostLogoutRedirectUri).ToArray();
            if (!removeTheseItems.IsNullOrEmpty())
            {
                await con.ExecuteAsync(
        ...
        public async Task<IEnumerable<ClientScopes>> GetClientScopeByClientId(int clientId)
        {
            await using var con = new SqlConnection(_connectionString);
            await con.OpenAsync();
        ...
        public async Task<IEnumerable<ClientScopes>> GetClientScopeByClientId(int clientId, IDbConnection con,
            IDbTransaction t)
        {
            return await con.QueryAsync<ClientScopes>(
                $"select * from {_options.DbSchema}.ClientScopes where ClientId=@ClientId", new {ClientId = clientId},
                commandTimeout: _options.CommandTimeOut, commandType: CommandType.Text, transaction: t);
        }
        ...
        private async Task UpdateClientScopeByClientId(IEnumerable<string> clientScopes, int clientId,
            IDbConnection con, IDbTransaction t)
        {
            ...
            await con.ExecuteAsync(
                $"delete from {_options.DbSchema}.ClientScopes where  ClientId = @ClientId and Scope in @Scopes",
                new {ClientId = clientId, Scope = removeTheseItems}, transaction: t);
        }

        public async Task<IEnumerable<ClientSecrets>> GetClientSecretByClientId(int ClientId)
        {
            await using var con = new SqlConnection(_connectionString);
            await con.OpenAsync();
        ...
        public async Task<IEnumerable<ClientSecrets>> GetClientSecretByClientId(int ClientId, IDbConnection con,
            IDbTransaction t)
        {
            return await con.QueryAsync<ClientSecrets>(
                $"select * from {_options.DbSchema}.ClientSecrets where ClientId=@ClientId", new {ClientId},
                commandTimeout: _options.CommandTimeOut, commandType: CommandType.Text, transaction: t);
        }
        ...
        private async Task UpdateClientSecretByClientId(IEnumerable<Secret> clientSecrets, int clientId,
            IDbConnection con, IDbTransaction t)
        {
            ...
            foreach (var item in updatedItems)
            {
                if (existingItems.Any(x => x.Type != item.Type || x.Value != item.Value))
                {
                    var insertSql =
                        ($"insert into {_options.DbSchema}.ClientSecrets (ClientId,Description,Expiration,Type,Value, Created) values (@ClientId,@Description,@Expiration,@Type,@Value, @Created)"
                        );
                ...
                else if (existingItems.Any(x => x.Type == item.Type && x.Value == item.Value))
                {
                    var updateSql =
                        ($"update {_options.DbSchema}.ClientSecrets set Description = @Description,Expiration = @Expiration where Value = @Value and Type = @Type and ClientId = @ClientId"
                        );
                    DynamicParameters insertParameters = new DynamicParameters();
                    insertParameters.Add("ClientId", clientId);
                    insertParameters.Add("Description", item.Description);
                    ...
                    insertParameters.Add("Type", item.Type);
                    insertParameters.Add("Value", item.Value);
                    await con.ExecuteAsync(updateSql, insertParameters, transaction: t);
                }
            }

            var removeTheseItems = existingItems
                .Where(c => updatedItems.All(f => f.Value != c.Value && f.Type != c.Type && c.Expiration != null && c.Expiration < DateTimeOffset.UtcNow)).ToList();
            foreach (var item in removeTheseItems)
            {
                await con.ExecuteAsync(
                    $"delete from {_options.DbSchema}.ClientSecrets where ClientId = @ClientId and [Value] = @Value and [Type]=@Type",
                    new {ClientId = clientId, item.Value, item.Type}, transaction: t);
            }
        }
        ...
        public async Task<IEnumerable<ClientClaims>> GetClientClaimByClientId(int clientId, IDbConnection con,
            IDbTransaction t)
        {
            return await con.QueryAsync<ClientClaims>(
                $"select * from {_options.DbSchema}.ClientClaims where ClientId=@ClientId", new
                {
                    ClientId = clientId
                }, commandTimeout: _options.CommandTimeOut, commandType: CommandType.Text, transaction: t);
        }

        public async Task UpdateClientClaimByClientId(IEnumerable<ClientClaim> clientClaims, int clientId)
        {
            await using var con = new SqlConnection(_connectionString);
            await con.OpenAsync();
            await using var t = await con.BeginTransactionAsync();
        ...
        private async Task UpdateClientClaimByClientId(IEnumerable<ClientClaim> clientClaims, int clientId,
            IDbConnection con, IDbTransaction t)
        {
            var dbItems = await GetClientSecretByClientId(clientId, con, t);
            var existingItems = dbItems.ToList();
            ...
            var removeTheseItems = existingItems
                .Where(c => updatedItems.All(f => f.Value != c.Value && f.Type != c.Type)).ToList();
            foreach (var item in removeTheseItems)
            {
                await con.ExecuteAsync(
                    $"delete from {_options.DbSchema}.ClientClaims where ClientId = @ClientId and [Value] = @Value and [Type]=@Type;",
                    new {ClientId = clientId, item.Value, item.Type}, transaction: t);
            }
        }

        public async Task<IEnumerable<ClientIdPRestrictions>> GetClientIdPRestrictionByClientId(int clientId)
        {
            await using var con = new SqlConnection(_connectionString);
            await con.OpenAsync();
        ...
        public async Task<IEnumerable<ClientIdPRestrictions>> GetClientIdPRestrictionByClientId(int clientId,
            IDbConnection con, IDbTransaction t)
        {
            return await con.QueryAsync<ClientIdPRestrictions>(
                $"select * from {_options.DbSchema}.ClientIdPRestrictions where ClientId=@ClientId",
                new {ClientId = clientId}, commandTimeout: _options.CommandTimeOut, commandType: CommandType.Text,
                transaction: t);
        }
        ...
        private async Task UpdateClientIdPRestrictionByClientId(IEnumerable<string> clientIdPRestrictions, int clientId,
            IDbConnection con, IDbTransaction t)
        {
            ...
            var removeTheseItems = existingItems.Where(c => !updatedItems.Exists(d => d == c.Provider))
                .Select(c => c.Provider).ToArray();
            if (!removeTheseItems.IsNullOrEmpty())
            {
                await con.ExecuteAsync(
        ...
        public async Task<IEnumerable<ClientCorsOrigins>> GetClientCorsOriginByClientId(int clientId, IDbConnection con,
            IDbTransaction t)
        {
            return await con.QueryAsync<ClientCorsOrigins>(
                $"select * from {_options.DbSchema}.ClientCorsOrigins where ClientId=@ClientId",
                new {ClientId = clientId}, commandTimeout: _options.CommandTimeOut, commandType: CommandType.Text,
                transaction: t);
        }
        ...
        private async Task UpdateClientCorsOriginByClientId(IEnumerable<string> clientCorsOrigins, int clientId,
            IDbConnection con, IDbTransaction t)
        {
            ...
            var removeTheseItems = existingItems.Where(c => !updatedItems.Exists(d => d == c.Origin))
                .Select(c => c.Origin).ToArray();
            if (!removeTheseItems.IsNullOrEmpty())
            {
                await con.ExecuteAsync(
        ...
        public async Task<IEnumerable<ClientProperties>> GetClientPropertyByClientId(int clientId, IDbConnection con,
            IDbTransaction t)
        {
            return await con.QueryAsync<ClientProperties>(
                $"select * from {_options.DbSchema}.ClientProperties where ClientId=@ClientId",
                new {ClientId = clientId}, commandTimeout: _options.CommandTimeOut, commandType: CommandType.Text,
                transaction: t);
        }
        ...
        private async Task UpdateClientPropertyByClientId(IDictionary<string, string> clientProperties, int clientId,
            IDbConnection con, IDbTransaction t)
        {
            ...
            foreach (var item in existingItems)
            {
                if (!updatedItems.ContainsKey(item.Key) && updatedItems[item.Key] != item.Value)
                {
                    await con.ExecuteAsync(
                        ...
                        new {ClientId = clientId, item.Value, item.Key}, transaction: t);
                }
            }
        }
        ...
        public async Task UpdateAsync(Client client)
        {
            ...
            try
            {
                var ret = await con.ExecuteAsync(
                    $"update {_options.DbSchema}.Clients set AbsoluteRefreshTokenLifetime = @AbsoluteRefreshTokenLifetime," +
                    ...
                    "UpdateAccessTokenClaimsOnRefresh=@UpdateAccessTokenClaimsOnRefresh where ClientId=@ClientId;",
                    entity, commandTimeout
                    :
                    _options.CommandTimeOut, commandType:
